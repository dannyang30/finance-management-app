...
function App() {
  const [transactions_data, setTransactions_data] = useState(default_transactions_data);

  useEffect(() => {
    fetch('http://localhost:8081/transactions')
    .then(res => res.json()) // this returns a Promise object, which represents the eventual completion (or failure) of an asynchronous operation, and its resulting value. 
  ...
  return(
    <>
      <div className="app-div">
          ...
          <TopSection/>
          <ChatbotSection/>
          <FinancialDetailsSection transactions_data={transactions_data}/>
